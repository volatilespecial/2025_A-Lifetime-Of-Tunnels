...
static void init(void) {
  ...
  _quad = gl4dgGenQuadf();
  initText(&_textTexId, 
  "Animation créée par Anparasan ANPUKKODY\n"
  "     Avec l'aide précieuse de Farès BELHADJ"
    );
}
...
static void initText(GLuint * ptId, const char * text) {
  ...
  if( !(font = TTF_OpenFont("SourceSerif4.ttf", 140)) ) {
    fprintf(stderr, "TTF_OpenFont: %s\n", TTF_GetError());
    return;
  }
  /* création d'une surface SDL avec le texte */
  d = TTF_RenderUTF8_Blended_Wrapped(font, text, c, 3200);
  if(d == NULL) {
    TTF_CloseFont(font);
