...
//    //gl4duOrthof(-2, 2, -2 * ratio, 2 * ratio, 2, 1000);
   viewportResize();
...
void walls_init(int nb_sections){
   ...
   static float angFourWalls = 0.0f;
   float transz = -3.0f;
   float rc = 1.0f;
   float bc = 1.0f;
   float gc = 1.0f;
...
void walls_draw(void){
   for(int i = currentWallReveal; i >= 0; --i){
      for(int j = 0; j<4; ++j){
         //gl4duLoadIdentityf();
...
void walls_Move(void) {
   //sleep(0.4);
  SDL_Delay(65);
   if(currentWallReveal < nb_sections - 1) currentWallReveal++;
}
