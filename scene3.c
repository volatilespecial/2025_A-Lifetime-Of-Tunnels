...
static void walls_Rotation(void);
...
static float currentWallRotation = 0.0f;
static float angleSection = 0.0f;
static int currentBackup = 0;
static int wallMove = 0;



void scene3(int state) {
...
void walls_init(int nb_sections){
   ...
   for(int i = 0; i<nb_sections; ++i){
      ...
      sections[i].setTexture = GL_FALSE;
...
void walls_Rotation(void) {
  //SDL_Delay(200);
  static float v = 0.1f;//0.005f;
  ...
  /* fin récup de dt */
   ...
   if(wallMove == 0){
      if(currentBackup < nb_sections){
         angleSection = currentWallRotation * 4.0f;
         for(int i = 0; i < 4; ++i){
            sections[currentBackup].walls[i].rot.angle = angleSection;
            angleSection+=90.0;
         }
         currentWallRotation+=10.0 * dt;//0.5f;
	 SDL_Delay(60);
         currentBackup++;
      }else{
         wallMove = 1;
